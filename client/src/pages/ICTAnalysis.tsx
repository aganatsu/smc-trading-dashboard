...
export default function ICTAnalysis() {
  ...
  return (
    <div className="flex h-full overflow-hidden">
      {/* Symbol selector sidebar — desktop only */}
      <div className="hidden md:block w-40 flex-shrink-0 bg-card border-r border-border overflow-y-auto">
      ...
      <div className="flex-1 overflow-y-auto min-w-0">
        {/* Mobile: Horizontal symbol selector */}
        <div className="md:hidden flex items-center gap-1 px-2 py-2 border-b border-border bg-card/50 overflow-x-auto flex-shrink-0">
          {SYMBOLS.map((sym) => (
            <button
              key={sym}
              onClick={() => setSelectedSymbol(sym)}
              className={`px-2 py-1 text-[10px] font-mono font-bold whitespace-nowrap rounded transition-colors ${
                selectedSymbol === sym
